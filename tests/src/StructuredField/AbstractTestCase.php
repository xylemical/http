...
abstract class AbstractTestCase extends TestCase {
   ...
   *
   * @return \Xylemical\Http\StructuredField\StructuredFieldTestCase[][]
   *   The test cases.
   */
   ...
   *   The bare item value.
   *
   * @return \Xylemical\Http\StructuredField\StructuredFieldItemInterface|null
   *   The bare item.
   */
   ...
   *   The construct.
   *
   * @return \Xylemical\Http\StructuredField\StructuredFieldInterface|null
   *   The result.
   */
  ...
  public function testParse(StructuredFieldTestCase $case): void {
    ...
    if ($raw = $case->getRaw()) {
      try {
        $parser = new Parser(
          new StructuredFieldTokenizer(),
          new StructuredFieldsLexer($case->getHeaderType())
        );
        $raw = $parser->parse(implode(', ', $raw));
        $rawString = $raw->serialize();
        $result = TRUE;
      }
